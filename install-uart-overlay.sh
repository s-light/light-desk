#!/bin/sh
# Build and install the light-desk UART-DMX device-tree overlay
# (overlays/k3-am62-pocketbeagle2-light-desk-uart-dmx.dtso), and wire it
...
#   ls -la /dev/ttyS1 /dev/ttyS3 /dev/ttyS4 /dev/ttyS5
#   sudo ~/light-desk/apply-ola-config.sh
#
...
#
# Usage: ./install-uart-overlay.sh
# Override the DTB source tree with:
#   DTB_SRC_DIR=/opt/source/dtb-X.Y.x ./install-uart-overlay.sh

set -eu

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
OVERLAY_NAME=k3-am62-pocketbeagle2-light-desk-uart-dmx
...
echo "    ls -la /dev/ttyS1 /dev/ttyS3 /dev/ttyS4 /dev/ttyS5"
echo "    sudo ~/light-desk/apply-ola-config.sh"
